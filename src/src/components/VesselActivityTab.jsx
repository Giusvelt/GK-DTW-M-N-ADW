...
export default function VesselActivityTab() {
    const { 
        activities, lastUpdate, loading, fetchActivities, productionPlans 
    } = useActivityStore();
    const { vessels } = useVesselStore();
    const { geofences } = useGeofenceStore();
    const { crewVesselId, companyVesselIds, profile: userProfile } = useData();
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        {[
                            ...
                            <motion.div 
                                ...
                                className="bg-white/80 backdrop-blur-md rounded-[2rem] p-5 lg:p-6 border border-white shadow-sm hover:shadow-md transition-all group overflow-hidden relative"
                            >
