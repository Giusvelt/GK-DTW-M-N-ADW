...
export default function ProductionTargetTab() {
    const { productionPlans, upsertPlan, deletePlan, activities, loading } = useActivityStore();
    const { vessels, updateVessel } = useVesselStore();
            ...
            <div className="tab-content production-targets-tab mt-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
                    <motion.div 
                        ...
                        className="bg-white rounded-2xl p-4 border border-white shadow-sm flex items-center gap-4"
                    >
                        ...
                        <div className="flex-1">
                            ...
                            <div className="h-1.5 w-full bg-surface-low/30 rounded-full overflow-hidden">
                                <motion.div 
                                    ...
                                    initial={{ width: 0 }}
                                    animate={{ width: `${totalTarget > 0 ? Math.min(100, (deliveredTotal / totalTarget) * 100) : 0}%` }}
                            ...
                            {(vessels || []).map((vessel, i) => {
                                ...
                                return (
                                    <motion.tr 
                                        ...
                                        transition={{ delay: Math.min(i * 0.05, 0.5) + 0.4 }}
                                        className="group"
                                    >
                                            ...
                                            <div className="flex flex-col gap-1 w-24">
                                                ...
                                                <div className="h-1 w-full bg-surface-low/30 rounded-full overflow-hidden">
                                                    <motion.div 
                                                        className="h-full bg-primary" 
                                                        initial={{ width: 0 }}
                                                        animate={{ width: `${Math.min(100, tripProgress)}%` }}
                                                        transition={{ duration: 0.8, ease: "easeOut", delay: 0.7 + (i * 0.05) }}
                                                    />
                                                </div>
                                            </div>
                                        </td>
